...
class FlashAttention(Cell):
    ...
    Inputs/outputs/signatures are identical to upstream. Casting (e.g., to FP16) is expected to be handled by the caller.
    """
    ...
    def __init__(
            ...
            sparse_mode=0,
            input_layout="TH",   # ignored here; we always run TND FA
            pa_kv_head_num=None,
            pa_mla_v_dim=0
    ):
        super().__init__()
        ...
        # TND FA for **prefill** (keeps caller-configured next_tokens/sparse_mode)
        ...
        # Decode kernel (PagedAttention) unchanged
        self.paged_attention = ops.auto_generate.PagedAttention(
            self.head_num, scale_value, pa_kv_head_num, mla_v_dim=pa_mla_v_dim)

        # --- helpers (no Tile; no casts on user tensors) ---
    ...
    def _kv_from_cache_tnd(self, cache, block_tables, actual_seq_kvlen):
        ...
        max_len_i32 = self._cast(max_len, mstype.int32)              # cast only the ReduceMax result
    ...
    def construct(self,
        ...
        if self.is_prefill:
            ...
            if has_chunk:
                ...
                # ----- Decode with PagedAttention (unchanged) -----
