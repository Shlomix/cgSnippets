...
// Division using reciprocal approximation and refinement
#define SIMD_DIV_FLOAT(x, y) ({ \
    float32x4_t _recip = vrecpeq_f32(y); \
    _recip = vmulq_f32(vrecpsq_f32(y, _recip), _recip); \
    _recip = vmulq_f32(vrecpsq_f32(y, _recip), _recip); \
    vmulq_f32(x, _recip); \
})
...
#pragma unroll
    for (size_t i = 0; i < span; ++i) {
        dst[i].data = SIMD_DIV(src_a_l[i].data, src_a_r.data);
    }
}
