...
class FlashAttention(Cell):
    ...
    def __init__(self,
        ...
        self._Mul        = ops.Mul()             # <-- use directly: self._Mul(x, y)
        self._Add        = ops.Add()
    ...
    def _gather_contiguous_kv(self, key_cache, value_cache,
        ...
        BS_t = ops.scalar_to_tensor(int(key_cache.shape[1]), mstype.int32)   # block_size tensor
        ...
        global_ix = self._Add(self._Mul(phys_blk, BS_t), offs)               # [B, max_kv_len_py]
        ...
        self._dbg("FA.gather k/v shapes:", str(k_contig.shape), "/", str(v_contig.shape),
                  " max_kv_len_py=", str(max_kv_len_py), " BS=", str(int(key_cache.shape[1])))
    ...
    def construct(self,
        ...
        if self.force_fa_chunk2p and self._is_second_plus_chunk(q_seq_lens):
            ...
            if ok:
                # Gather contiguous KV using Python ints for shape
