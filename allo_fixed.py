...
class PageAllocator:
    def __init__(
        ...
        self._decode_has_spilled_to_remote = False
    ...
    def _alloc_pages(self, total_pages: int, spill_to_remotes: bool) -> List[Dict]:
        ...
        if self.locals_present and not self._decode_has_spilled_to_remote:
            while len(self.local_tables) < local_table_cap:
                table_id = self.alloc_table(-1)
                self.local_tables.append((table_id, []))
                self.table_id_to_worker[table_id] = -1

        if not self._decode_has_spilled_to_remote:
        ...
        if self.remote_worker_ids and (spill_to_remotes or not self.locals_present):
            for wid in self.remote_worker_ids:
                if not self.remote_tables[wid]:
                    if not self._decode_has_spilled_to_remote:
                        self._decode_has_spilled_to_remote = True
        ...
        while len(pages) < total_pages:
            ...
            # 🚫 Block local tables if we’ve spilled already
            if worker_id == -1 and self._decode_has_spilled_to_remote:
                continue
            ...
            if len(current_table) >= self.pages_per_table:
                if worker_id == -1:
                    if self._decode_has_spilled_to_remote:
                        continue
            ...
            if worker_id != -1 and not self._decode_has_spilled_to_remote:
                self._decode_has_spilled_to_remote = True
            visited.clear()
