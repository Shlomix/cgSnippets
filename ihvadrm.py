...
class FlashAttention(Cell):
    ...
    Behavior
      ...
      • Second+ prefill (has prior ctx): FlashAttention (BSH) over full prefix by gathering dense KV from paged cache.
        (PoC supports batch=1; else fallback to PA.)
    ...
    def _repeat_kv_to_q_heads(self, dense_kv):
        ...
        # repeat by broadcasting concat instead of tile to avoid dynamic reps
        x = ops.concat([x] * rep, axis=1)
    ...
    def _sanitize_fa_inputs(self, q_bsh, k_bsh, attn_mask, aql, akl):
        ...
        if attn_mask is None:
            ...
            if am.shape[0] == 1 and B > 1:
                am = ops.broadcast_to(am, (B, am.shape[1], am.shape[2]))
            if am.dtype != qdtype:
    ...
    def _dense_kv_from_cache_b1(self, key_cache, value_cache, block_tables, kv_end_i32):
        """
        Gather dense KV from paged cache for B=1 using safe indexing.
        key_cache: [num_blocks, block_size, kv_heads*head_dim] or [num_blocks, block_size, kv_heads, head_dim]
        block_tables: [1, num_blocks] int32 with physical block ids (may include -1 in unused tail).
        kv_end_i32: scalar int32 (prefix length to include, <= num_blocks*block_size).
        Returns: K_dense, V_dense TH: [T_kv, H_kv], and block_size as Python int.
        """
        ...
        if len(shape) == 4:
            ...
            # fallback reshape to raise a clear error if shapes are unexpected
            nb, bs, H_kv = int(shape[0]), int(shape[1]), int(shape[-1])
            kc_flat = self._Reshape(key_cache, (nb * bs, H_kv))
            vc_flat = self._Reshape(value_cache, (nb * bs, H_kv))

        # used_blocks = min(num_blocks, ceil(kv_end/bs))
        ...
        # take first used_blocks physical IDs
        # build range indices [0..used_blocks)
        ...
        # sanitize ids: clamp to [0, nb-1] to avoid -1 or OOB
        zero = ops.scalar_to_tensor(0, mstype.int32)
        nbm1 = ops.scalar_to_tensor(nb - 1, mstype.int32)
        ids = ops.maximum(ids, zero)
        ids = ops.minimum(ids, nbm1)    # [used_blocks]

        # Build per-token indices without dynamic tile:
        # tok_idx2d = ids[:,None]*bs + arange(bs)[None,:] -> [used_blocks, bs]
        ids_2d = self._Reshape(ids, (-1, 1))          # [u,1]
        base = self._Mul(ids_2d, bs_t)                # [u,1]
        offs = self._Range(zero, bs_t, ops.scalar_to_tensor(1, mstype.int32))  # [bs]
        offs = self._Reshape(offs, (1, bs))           # [1,bs]
        tok2d = base + offs                           # broadcast -> [u,bs]
        tok_idx = self._Reshape(tok2d, (-1,))         # [u*bs]
        tok_idx = tok_idx[:kv_end_i32]                # trim to exact kv_end
    ...
    def construct(self,
        ...
        if self.is_prefill:
            ...
            # safe dense KV gather from cache (B=1)
