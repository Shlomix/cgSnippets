# ===== Graph-safe FA prefill block (Option B / external caches) =====
from mindspore import ops, Tensor, dtype as mstype

# Ensure routing tensors are int32
...
if use_fa_prefill and (key_cache is not None) and (value_cache is not None):
    kc, vc = key_cache, value_cache  # external caches

    # ---------- gather only valid KV blocks ----------
    B  = int(block_tables.shape[0])
    M  = int(block_tables.shape[1])
    bs = int(kc.shape[1])                  # block_size
    Hk = int(kc.shape[-1])

    # m_per_batch = ceil(L/bs)
    L  = batch_valid_length                                           # [B] i32
    m_per_batch = ops.floor_div(ops.add(L, Tensor(bs - 1, mstype.int32)),
                                Tensor(bs, mstype.int32))             # [B] i32

    idx = ops.cast(ops.arange(0, M, 1), mstype.int32)                 # [M] i32
    ...
    # ---------- normalize Q to BSH & define symbols up-front ----------
    ...
    if q_bsh is not None:
        ...
        # quick guards to avoid undefined symbols / illegal reshape
        d = 0
        if nh > 0 and (Hq % nh == 0):
            d = Hq // nh

        # ---------- handle GQA by tiling KV heads ----------
        if d > 0 and (Hk % d) == 0:
            kv_heads = Hk // d
            if kv_heads != nh:
        ...
        # ---------- call FA only when dims line up ----------
        if d > 0 and (Hk == Hq):
...
# ===== end FA prefill block; fall through to your original branches if not returned =====
