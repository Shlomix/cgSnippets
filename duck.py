...
class FlashAttention(Cell):
    ...
    def __init__(
        ...
        # Original FA (TH layout, unchanged path)
        ...
        # Extra FA for 2D(+)-chunk prefill (TND layout; causal; next_tokens=0)
        self._flash_attention_tnd = FlashAttentionScore(
            head_num=head_num,
            keep_prob=keep_prob,
            scale_value=scale_value,
            pre_tokens=pre_tokens,   # already very large by default
            next_tokens=0,
            inner_precise=0,
            input_layout="TND",
            sparse_mode=3)
    ...
    def _has_tnd_ragged(self, actual_seq_qlen, actual_seq_kvlen):
        # TND requires both cumulative arrays; do not rely on is_prefill flag.
    ...
    def construct(self,
        ...
        # Keep original cache write behavior (some pipelines flip is_prefill during chunk prefill).
        ...
        if self._has_tnd_ragged(actual_seq_qlen, actual_seq_kvlen):
            ...
            # TND FA; prefix must be None; next_tokens fixed in primitive.
