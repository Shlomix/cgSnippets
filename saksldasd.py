...
class FlashAttention(Cell):
    ...
    2nd+ chunk       → (optional) gather contiguous KV → FlashAttention(query, Kc, Vc)
    decode / others  → PagedAttention

    Env:
      MF_FORCE_FA_CHUNK=1  → enable FA for 2nd+ chunk prefill (default: 1)
      MF_DEBUG_ATTENTION=1 → enable prints (default: 1)
    """
    ...
    def __init__(self,
        ...
        self._Cond       = ops.cond  # lazy branch selection (only one branch runs)
    ...
    def _classify_step(self, q_seq_lens):
        ...
        return is_prefill_t, second_plus_chunk_t, is_decode_t
    ...
    def _gather_contiguous_kv(self, key_cache, value_cache, block_tables, batch_valid_length, q_seq_lens, bsz):
        ...
        # zero masked tail; FA also receives lengths
    ...
    def _run_fa_chunk(self, operands):
        ...
        bsz = query.shape[0]
        self._dbg("FA path: FlashAttention (2nd+ chunk)")
    ...
    def _run_paged(self, operands):
        ...
        self._dbg("FA path: PagedAttention")
    ...
    def construct(self,
        ...
        # Always write new tokens into cache (unless MLA)
        ...
        # classify
        is_prefill_t, second_plus_chunk_t, _ = self._classify_step(q_seq_lens)
        ...
        # ---- Non-prefill: choose between FA(2nd+) and PagedAttention lazily ----
        # Only one branch runs thanks to ops.cond
        if (not self.use_multi_latent_attention) and self.force_fa_chunk2p:
            ...
            out_ctx = self._Cond(second_plus_chunk_t,
                                 self._run_fa_chunk,
                                 self._run_paged,
                                 operands)
            return out_ctx
        ...
        # default non-prefill
        return self._run_paged((query, key_cache, value_cache, block_tables, batch_valid_length,
                                q_seq_lens, actual_seq_qlen, actual_seq_kvlen, padding_mask, attn_mask))
