...
# ============================================================================
"""Flash Attention Layer with x16 gating and safe fallback to PagedAttention."""
...
class FlashAttention(Cell):
    ...
    2nd+ chunk prefill:
        - if MF_FORCE_FA_CHUNK=1 and x16-aligned:
              gather contiguous KV from cache -> FlashAttention
          else:
              PagedAttention
    decode:
        - PagedAttention
    ...
    def __init__(self,
        ...
        self.use_cond         = os.getenv("MF_USE_COND", "1") == "1"
    ...
    def _b(self, v: bool):
        return ops.scalar_to_tensor(bool(v), mstype.bool_)
    ...
    def _fa_ok_first_prefill(self, query, key):
        # Python-level ints (shape known at compile time)
        sq = query.shape[1]
        sk = key.shape[1]
    ...
    def _fa_ok_chunk2p(self, q_seq_lens, batch_valid_length):
        ...
        kv_ok = self._ReduceAll(self._Equal(self._Mod(kv_end, ops.scalar_to_tensor(16, mstype.int32)),
                                            ops.scalar_to_tensor(0,  mstype.int32)))
        ok = self._LogicalAnd(q_ok, kv_ok)
    ...
    def _gather_contiguous_kv(self, key_cache, value_cache, block_tables, batch_valid_length, q_seq_lens, bsz):
        ...
        global_ix = self._Add(self._Mul(phys_blk, BS), offs)                # [B, max_kv_len]
    ...
    # ---------- branch bodies ----------
    def _run_fa_chunk(self, operands):
        (query, key_cache, value_cache, block_tables, batch_valid_length,
         q_seq_lens, actual_seq_qlen, actual_seq_kvlen, padding_mask, attn_mask) = operands
        bsz = query.shape[0]
    ...
    def _run_paged(self, operands):
        (query, key_cache, value_cache, block_tables, batch_valid_length,
         q_seq_lens, _actual_seq_qlen, _actual_seq_kvlen, _padding_mask, attn_mask) = operands
    ...
    def construct(self,
                  ...
                  key_cache=None,
                  value_cache=None):
        # Always write K/V into cache (unless MLA)
        ...
        # ---- Non-prefill ----
        # decode or 2nd+ chunk
        is_chunk2p = self._is_second_plus_chunk(q_seq_lens)
        operands = (query, key_cache, value_cache, block_tables, batch_valid_length,
                    q_seq_lens, actual_seq_qlen, actual_seq_kvlen, padding_mask, attn_mask)

        # Decide: FA on 2nd+ chunk only if user enabled and x16 holds; else Paged
        fa_ok = self._fa_ok_chunk2p(q_seq_lens, batch_valid_length)
        pred = self._LogicalAnd(self._b(self.force_fa_chunk2p),
                                self._LogicalAnd(is_chunk2p, fa_ok))

        if (not self.use_multi_latent_attention) and self.force_fa_chunk2p:
            if self.use_cond:
                return self._Cond(pred, self._run_fa_chunk, self._run_paged, operands)
            # compute-both-and-select fallback (more compute, very robust)
            fa_ctx    = self._run_fa_chunk(operands)
            paged_ctx = self._run_paged(operands)
            return self._Select(pred, fa_ctx, paged_ctx)

        return self._run_paged(operands)
